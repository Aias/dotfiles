...
- **Diagnose before acting.** For a standalone failure report or a question about behavior, diagnose and present evidence before making changes unless a fix is already authorized. When a failure concerns work you are implementing or have delivered in the current task, continue through the scoped fix and validation under the existing authorization. An explicit request for diagnosis only still limits the task to diagnosis. Propose a plan and wait for confirmation before any destructive or hard-to-reverse workflow; in unattended runs, the approved request counts as confirmation, pausing only at destructive gates. If an approved approach starts ballooning mid-execution — the diff sprawls past its expected size, new cases keep surfacing, the abstraction fights you — treat that as a signal the plan is wrong: stop and re-check the problem against the plan rather than pushing on.
...
<!-- harness: codex -->
Codex: Astra (`gpt-6-astra`) for judgment, Sol (`gpt-5.6-sol`) for substantive coding, Terra (`gpt-5.6-terra`) for bounded implementation, Luna (`gpt-5.6-luna`) for retrieval.
...
- Work on a feature branch, with a dedicated worktree for independent cross-repo edits. Use SSH URLs for cloning. Amend commits only when explicitly requested. Present commit summaries oldest first and name actual branches when explaining conflicts.
...
- **Never add code comments.** Exceptions: very strong precedent in the surrounding code shows comments are required, or the user explicitly requests them. The mere presence of nearby comments is not enough. Prefer clear names and structure. Do not proactively delete existing comments unless rewriting the code they describe. Touching the same file or reviewing a change does not justify comment removal. When rewriting associated code, keep, update, or remove its comments according to their accuracy and usefulness. Machine-read directives (shebangs, codegen pragmas) are code, not comments. Add `TODO` (or similar) markers only when the user specifically requests them.
...
<!-- BEGIN COMPILED -->
Code Quality|skills/change-review|No shipped stubs, mocks, hardcoded fixtures, or "temporary" literals. Replace stand-ins with real sources before handoff. Mid-stream stubs stay greppable through a stub/mock-prefixed identifier, never a comment:L95|Hard rule: a file may not cross from below 1000 lines to above. Only waiver is extremely uniform content (data table, generated code, flat enum) where any split would hurt readability. Decompose first by default:L102|No barrel files (index.ts re-exports). Import directly from source modules:L170|Prefer native semantic elements. Preserve keyboard interaction and visible focus when building or simplifying UI:L185|Style selected, active, and expanded states through data or appropriate ARIA attributes and CSS selectors rather than conditional class names:L203
