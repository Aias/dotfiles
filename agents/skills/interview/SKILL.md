...
name: interview
description: >
  Interview the user to stress-test a plan or clarify requirements before implementation, then record the decisions. Use when the user requests an interview or design-questioning session.
---
...
- **Recommend an answer for every question.** State your pick and why in one line, then ask. In the question tool, put your recommendation first and append "(Recommended)" to the label. Freeform questions: lead with "I'd suggest X because Y — does that hold, or are you thinking differently?"
- **Keep freeform answers cheap.** A sentence or two max. If the answer would be longer, restructure as multiple choice.

The interview is done when the material decisions are settled and the remaining assumptions are explicit. Then write the findings into the existing plan or spec document; if none exists, create one.
