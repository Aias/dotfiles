...
- **Challenge the premise before building on it.** When the request embeds a proposed solution or design choice, verify it like any other claim: confirm the codebase doesn't already provide the capability, and that the proposed key, flag, or field actually distinguishes the entities it must (a per-tenant id can't drive a per-user rollout). Implementing a flawed premise faithfully is still a wrong answer.
...
- **Is "it works" true in the environment that matters?** A guard, flag, or config keyed to one environment is a premise to re-verify in each target — local, preview, production. A merged fix whose symptom still appears somewhere is a hypothesis, not a conclusion: trace the full delivery path to that environment (build and bundle caching, stale loaded clients) before declaring it fixed.
